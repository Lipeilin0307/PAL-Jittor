# -*- coding: utf-8 -*-
"""
评测指标：mIoU / nIoU / PD / FA
（PAL 原版 components/metric_new_crop.py 的框架无关迁移版）。

计算逻辑与原版逐行一致（numpy/skimage 实现，阈值 0.5，np.spacing(1) 作 eps，
PD/FA 用 skimage measure.label(connectivity=2) 8 连通 + 质心欧氏距离 < 3 判命中）。
唯一改动：输入从 torch.Tensor 泛化为 numpy / jittor.Var / torch.Tensor 均可，
经 `_to_numpy` 统一转成 numpy 后再计算；`update()` 的数值语义不受影响。

- SigmoidMetric.get()          -> (pixAcc, mIoU)，mIoU 为全数据集 Σinter/Σunion
- SamplewiseSigmoidMetric.get()-> (IoU_arr, nIoU)，逐样本 IoU 再取平均
- PD_FA_2.get(img_num)         -> (FA, PD)；FA 报告时通常 ×1e6
"""
import numpy as np
...
def _to_numpy(x):
    """把 numpy / jittor.Var / torch.Tensor 统一转成 numpy.ndarray。"""
    if isinstance(x, np.ndarray):
        return x
    if hasattr(x, 'cpu') and hasattr(x, 'detach'):  # torch.Tensor
...
class SigmoidMetric():
    ...
    def batch_intersection_union(self, output, target):
        ...
        maxi = 1  # nclass
        nbins = 1  # nclass
        predict = (_to_numpy(output) > self.score_thresh).astype('int64')  # P
        target = (_to_numpy(target) > self.score_thresh).astype('int64')  # T
...
class PD_FA_2():
    ...
    def update(self, preds, labels):
        ...
        predits  = (_to_numpy(preds) > 0.5).astype('int64')
        ...
        labelss = (_to_numpy(labels) > 0.5).astype('int64') # P
...
if __name__ == '__main__':
    pred = np.random.rand(8, 1, 512, 512)
    target = np.random.rand(8, 1, 512, 512)
    m1 = SigmoidMetric()
    ...
    pixAcc, mIoU = m1.get()
    _, nIoU = m2.get()
    print("pixAcc:", pixAcc, "mIoU:", mIoU, "nIoU:", nIoU)
